...
class VBTStateMachine {
  ...
  reset() {
    this.state = {
      ...
      hasVisitedHinge: false, // NEW: Biomechanical memory
  ...
  update(pose, timestamp, ctx, canvas) {
    ...
    // TRACK HINGE VISIT: Did the bell go below the hips at any point?
    if (wrist.y > hip.y + 0.05) this.state.hasVisitedHinge = true;

    const hinged = Math.abs(shoulder.y - hip.y) < this.THRESHOLDS.HINGE;
    const atRack = Math.abs(wrist.y - shoulder.y) < 0.15 && Math.abs(wrist.x - (smoothedPose.LEFT.SHOULDER.x + smoothedPose.RIGHT.SHOULDER.x)/2) < 0.2;

    if (atRack && !hinged) {
      if (++this.state.rackFrameCount >= this.THRESHOLDS.RACK_LOCK_FRAMES) this.state.currentPose = "RACK";
    } else {
      this.state.rackFrameCount = 0;
      if (wrist.y > (smoothedPose[side].KNEE.y + this.THRESHOLDS.KNEE_CROSS)) this.state.currentPose = "HINGE";
      else this.state.currentPose = "NONE";
    }
    ...
    if (this.state.phase === "IDLE") {
      ...
      if (nearlyStopped && isAtShoulder && !hinged) {
        if (++this.state.shoulderHoldFrames >= this.THRESHOLDS.CLEAN_HOLD_FRAMES) {
            result = this.classify(wrist, shoulder, hip, nose, hinged, this.state.hipCrossedUpward, true);
            this.state.phase = "LOCKED";
        }
      } else if (nearlyStopped) {
        result = this.classify(wrist, shoulder, hip, nose, hinged, this.state.hipCrossedUpward, false);
        this.state.phase = "LOCKED";
      }

      if (result) {
          result.velocity = this.state.currentRepPeak;
          this.state.hasVisitedHinge = false; // Reset memory after successful rep
      }
    } else if (this.state.phase === "LOCKED") {
      // Rep ends: Wait for the bell to move again before allowing next cycle
      if (Math.abs(this.state.smoothedVy) > this.THRESHOLDS.PULL_VELOCITY_TRIGGER) this.state.phase = "IDLE";
    }
    return result;
  }

  classify(w, s, h, nose, hinged, crossed, heldAtShoulder) {
    const isOverhead = w.y < (nose.y - 0.05); 
    const isAtShoulder = w.y <= (s.y + 0.12) && w.y >= (s.y - 0.08);
    
    if (this.state.hasVisitedHinge && crossed) {
      if (isOverhead) return { type: "SNATCH" };
      if (heldAtShoulder && isAtShoulder && !hinged) return { type: "CLEAN" };
      return { type: "SWING" };
    } 
    else if (isOverhead) return { type: "PRESS" };
    return null;
  }
