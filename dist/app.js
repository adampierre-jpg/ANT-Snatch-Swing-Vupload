...
async function initializeApp() {
    ...
    if (btnStartTest)
        btnStartTest.onclick = toggleTest;
    ...
    const visionGen = await FilesetResolver.forVisionTasks("https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@latest/wasm");
...
async function toggleTest() {
    ...
    if (!state.isTestRunning) {
        ...
        if (btnStartTest)
            btnStartTest.textContent = "Pause Test";
        if (btnReset)
        ...
        if (btnStartTest)
            btnStartTest.textContent = "Resume Test";
...
async function masterLoop(timestamp) {
    requestAnimationFrame(masterLoop);
    if (!state.isModelLoaded || !state.video)
        return;
...
// ============================================
function checkStartCondition(pose, _timeMs) {
    if (state.testStage !== "IDLE")
        return;
...
function checkEndCondition(pose, timeMs) {
    ...
    if (grace < CONFIG.RESET_GRACE_MS_AFTER_LOCK)
        return;
...
function runPhysics(pose, timeMs) {
    ...
    if (!state.canvas || !state.lockedCalibration)
        return;
    ...
    if (state.testStage === "RUNNING") {
        ...
        if (velEl)
            velEl.textContent = state.lastSpeed.toFixed(2);
    }
}
// ============================================
...
function runMovementLogic(pose) {
    ...
    if (state.phase === "IDLE" || state.phase === "LOCKOUT") {
        if (zone === 'FLOOR') {
            ...
            if (CONFIG.DEBUG_MODE)
                console.log("Phase: BOTTOM (from FLOOR)");
        }
            ...
            state.currentRepPeakWristX = wrist.x;
            if (CONFIG.DEBUG_MODE)
                console.log("Phase: BOTTOM (from RACK)");
        }
        // ADD THIS - detect press starting from rack
            ...
            state.currentRepPeak = 0;
            if (CONFIG.DEBUG_MODE)
                console.log("Phase: CONCENTRIC (press from rack)");
        }
    }
    ...
    // PHASE: CONCENTRIC - Tracking upward movement
        ...
        if (v > state.currentRepPeak)
            state.currentRepPeak = v;
...
// ============================================
function recordClean(_pose, cleanType) {
    const cleanData = {
        type: cleanType,
...
function updateMovementDisplay(movementType) {
    ...
    if (!movementEl || !configEl || !handsEl || !statusIndicator)
        return;
...
function drawDot(landmark, big, color) {
    if (!landmark || !state.ctx || !state.canvas)
        return;
